...
def RS_232():
    ...
    if PLC_ready == 'ready' and PLC_flag == 0 and plc_check_time >=3:
            ...
            if check_result == 1:
                ...
                if PLC_flag == 1:
                    ...
                    if PassOrNG == 1:
                        ...
                        result_label.place(x=screen_width * (1400 / tk_width), y=screen_height * (760 / tk_height))
...
def judgefunc():
    ...
    if sum == len(circle_center_list):
        ...
        rivet_label.place(x=screen_width * (1050 / tk_width),
                          y=(screen_height / 3) + screen_height * (430 / tk_height) + (0 * screen_height * (80 / tk_height)), relx=0.01, rely=0.01)
        ...
        if len(rivet_empty_list) != 0:
            for i in range(len(rivet_empty_list)):
                ...
                cv2.putText(ng_frame, '%d' % (i + 1), (rivet_empty_list[i][0] - 13 , rivet_empty_list[i][1] - 13), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1, cv2.LINE_AA)
...
def RivetDetect_Big(frame):
    ...
    if rivet_detect == True:
        ...
        small_circles = cv2.HoughCircles(img_gray, cv2.HOUGH_GRADIENT, 1, 13, param1=110, param2=8, minRadius=2, maxRadius=4)
...
def check_setting():
    global check_set, store_local_location, pathname, store_server_location
    global ser, HOST, PORT
    global circle_center_list, circle_center_area_list
    global check_local_path, check_server_path
