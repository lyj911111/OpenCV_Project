...
def decode(im):
    ...
    cv2.imwrite(store_location + "barcode0.png", im)
...
def read_frame():
    ...
    global PLC_sensor, folder_name, Serial_No, check_click_state
    ...
    global accum, count_pass_rivet, count_fail_rivet
    ...
    #cv2.imshow("frame_raw", frame_cp)
    if check_set == False and tact_time_flag == 0:
        start_time = time.time()
        tact_time_flag = 1

    if check_set == False:
        if check_click_state == False:
            click(250, 578)
            check_click_state = True
            #RV_SN.delete(0, END)
        ...
        if Serial_No != '':
            ...
            if check_result == 1:
                ...
                if PassOrNG == 1:
                    cv2.imwrite(store_location + "%s/rivet/pass/%s.png" % (folder_name, Serial_No), pass_frame)
                    ...
                    cv2.imwrite(store_location + "%s/rivet/fail/%s.png" % (folder_name, Serial_No), ng_frame)
...
def browse_folder():
    ...
    store_location = browse.dirName
    datapath.insert(20, store_location)
    browse.destroy()
...
def setting_window():
    ...
    set.geometry("{}x{}+{}+{}".format(int(screen_width * (830 / tk_width)), int(screen_height * (950 / tk_height)), \
                                      int((screen_width / 2) - int(screen_width * (830 / tk_width) / 2)),
                                      int((screen_height / 2) - int(screen_height * (950 / tk_height) / 2)) - 50))
    # set.geometry("800x600")
    set.title("Setting Window")
    set.configure(bg="#ebebeb")

    Label(set, text="초기 설정창", font="Helvetica 15 bold", bg="#ebebeb", bd=2, width=int(screen_width * (20 / tk_width)), \
    ...
    Label(set, text="ROI 설정(Big_Rivet)", height=int(screen_height * (2 / tk_height)),
    ...
    Label(set, text="ROI 설정(Small_Rivet)", height=int(screen_height * (2 / tk_height)),
          width=int(screen_width * (20 / tk_width)), fg="red", relief="groove", bg="#ebebeb",
          font="Helvetica 13 bold").place(x=screen_width * (505 / tk_width), y=screen_height * (55 / tk_height),
                                          relx=0.01, rely=0.01)

    Label(set, text="예외지역 설정", height=int(screen_height * (2 / tk_height)), width=int(screen_width * (20 / tk_width)),
          fg="red", relief="groove", bg="#ebebeb",
          font="Helvetica 13 bold").place(x=screen_width * (95 / tk_width), y=screen_height * (380 / tk_height),
                                          relx=0.01, rely=0.01)

    Label(set, text="바코드 위치 설정", height=int(screen_height * (2 / tk_height)), width=int(screen_width * (20 / tk_width)),
          fg="red", relief="groove", bg="#ebebeb",
          font="Helvetica 13 bold").place(x=screen_width * (505 / tk_width), y=screen_height * (385 / tk_height),
                                          relx=0.01, rely=0.01)

    Label(set, text="데이터 저장 경로 설정", font="Helvetica 12 bold", fg="red", bg="#ebebeb", bd=2,
    ...
    datapath = Entry(set, width=int(screen_width * (55 / tk_width)), relief="groove", font="Helvetica 20 bold")
    datapath.place(x=0, y=screen_height * (870 / tk_height), relx=0.001, rely=0)
    ...
    Button(set, text="리벳 감지 시작 버튼", font="Helvetica 10 bold", relief="raised", overrelief="solid", bg="#ebebeb", \
           width=int(screen_width * (20 / tk_width)), height=int(screen_height * (2 / tk_height)), bd=3, padx=2, pady=2,
           command=start_rivet_detect).place(x=screen_width * (430 / tk_width), y=screen_height * (815 / tk_height))

    Button(set, text="리벳 재감지 버튼", font="Helvetica 10 bold", relief="raised", overrelief="solid", bg="#ebebeb", \
           width=int(screen_width * (20 / tk_width)), height=int(screen_height * (2 / tk_height)), bd=3, padx=2, pady=2,
           command=restart_rivet_detect).place(x=screen_width * (630 / tk_width), y=screen_height * (815 / tk_height))

    Button(set, text="...(Browse)", font="Helvetica 10 bold", relief="raised", overrelief="solid", bg="#ebebeb", \
           width=int(screen_width * (10 / tk_width)), height=int(screen_height * (1 / tk_height)), bd=3, padx=2, pady=2,
           command=browse_folder).place(x=screen_width * (260 / tk_width), y=screen_height * (828 / tk_height))

    ### 통신 방식
    ### Label
    Label(set, text="통신 방식", height=int(screen_height * (4 / tk_height)), width=int(screen_width * (10 / tk_width)),
          fg="red", relief="groove", bg="#ebebeb",
          font="Helvetica 13 bold").place(x=screen_width * (5 / tk_width), y=screen_height * (713 / tk_height),
                                          relx=0.01, rely=0.01)
    ...
    ### ROI(Big) Button
    Big_ROI_B = Button(set, text="ROI 추가", font="Helvetica 10 bold", relief="raised", overrelief="solid", bg="#ebebeb", \
                       width=int(screen_width * (15 / tk_width)), height=int(screen_height * (2 / tk_height)), bd=3,
                       padx=2, pady=2, command=add_Big_ROI)
    Big_ROI_B.place(x=screen_width * (30 / tk_width), y=screen_height * (300 / tk_height))

    Big_ROI_SC_B = Button(set, text="ROI 설정 완료", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                          bg="#ebebeb", \
                          width=int(screen_width * (15 / tk_width)), height=int(screen_height * (2 / tk_height)), bd=3,
                          padx=2, pady=2, command=set_comp_Big_ROI)
    Big_ROI_SC_B.place(x=screen_width * (245 / tk_width), y=screen_height * (300 / tk_height))

    ### ROI(Small) Button
    Small_ROI_B = Button(set, text="ROI 추가", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                         bg="#ebebeb", \
                         width=int(screen_width * (15 / tk_width)), height=int(screen_height * (2 / tk_height)), bd=3,
                         padx=2, pady=2, command=add_small_ROI)
    Small_ROI_B.place(x=screen_width * (440 / tk_width), y=screen_height * (300 / tk_height))

    Small_ROI_SC_B = Button(set, text="ROI 설정 완료", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                            bg="#ebebeb", \
                            width=int(screen_width * (15 / tk_width)), height=int(screen_height * (2 / tk_height)),
                            bd=3, padx=2, pady=2, command=set_comp_Small_ROI)
    Small_ROI_SC_B.place(x=screen_width * (655 / tk_width), y=screen_height * (300 / tk_height))
    ...
    B_ROI_B = Button(set, text="Barcode Area 설정", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                     bg="#ebebeb", \
                     width=int(screen_width * (15 / tk_width)), height=int(screen_height * (2 / tk_height)), bd=3,
                     padx=2, pady=2, command=set_Bar_area)
    B_ROI_B.place(x=screen_width * (440 / tk_width), y=screen_height * (630 / tk_height))

    B_SC_B = Button(set, text="Barcode Area 재설정", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                    bg="#ebebeb", \
                    width=int(screen_width * (15 / tk_width)), height=int(screen_height * (2 / tk_height)), bd=3,
                    padx=2, pady=2, command=reset_Bar_area)
    B_SC_B.place(x=screen_width * (655 / tk_width), y=screen_height * (630 / tk_height))
    ...
    ### Except Button
    Big_Except_B = Button(set, text="추가\n하기", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                          bg="#ebebeb", \
    ...
    Small_Except_B = Button(set, text="추가\n하기", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                            bg="#ebebeb", \
                            width=int(screen_width * (6 / tk_width)), height=int(screen_height * (5 / tk_height)), bd=3,
    ...
    Big_Except_SC_B = Button(set, text="설정\n완료", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                             bg="#ebebeb", \
                             width=int(screen_width * (6 / tk_width)), height=int(screen_height * (5 / tk_height)),
                             bd=3, padx=2, pady=2, command=set_comp_big_except)
    Big_Except_SC_B.place(x=screen_width * (340 / tk_width), y=screen_height * (460 / tk_height))

    Small_Except_SC_B = Button(set, text="설정\n완료", font="Helvetica 10 bold", relief="raised", overrelief="solid",
                               bg="#ebebeb", \
                               width=int(screen_width * (6 / tk_width)), height=int(screen_height * (5 / tk_height)),
                               bd=3, padx=2, pady=2, command=set_comp_small_except)
    Small_Except_SC_B.place(x=screen_width * (340 / tk_width), y=screen_height * (580 / tk_height))
...
def main():
    ...
    global RV_SN, RV_TIME, RV_ACC, RV_PASS, RV_NG, RV_TACT
    # global SN_label
    # global image_label, set, datapath
    ...
    Label(root, text="저장 폴더 열기\nOpen folder", height=int(screen_height * (5 / tk_height)),
          width=int(screen_width * (13 / tk_width)), fg="red", relief="groove", bg="#ebebeb",
          font="Helvetica 13 bold").place(x=screen_width * (970 / tk_width),
                                          y=(screen_height / 3) + screen_height * (140 / tk_height) + (
                                                      0 * screen_height * (80 / tk_height)), relx=0.01, rely=0.01)

    Label(root, text="바코드 이미지\nBarcode Image", height=int(screen_height * (5 / tk_height)),
          width=int(screen_width * (13 / tk_width)), fg="red", relief="groove", bg="#ebebeb",
          font="Helvetica 13 bold").place(x=screen_width * (970 / tk_width),
                                          y=(screen_height / 3) + screen_height * (270 / tk_height) + (
                                                      0 * screen_height * (80 / tk_height)), relx=0.01, rely=0.01)

    Label(root, text="결과\nResult", height=int(screen_height * (5 / tk_height)),
          width=int(screen_width * (13 / tk_width)), fg="red", relief="groove", bg="#ebebeb",
    ...
    Button(root, text="폴더 열기\nOpen Folder", font="Helvetica 10", relief="raised", overrelief="solid", bg="#ebebeb", \
    ...
    Button(root, text="-90도 회전", font="Helvetica 10", relief="raised", overrelief="solid", bg="#ebebeb", \
           width=int(screen_width * (10 / tk_width)), height=int(screen_height * (4 / tk_height)), bd=3, padx=2, pady=2,
           command=roate_left) \
