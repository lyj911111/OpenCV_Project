...
########################## 원 탐지 부분 ################################
det_cir = img.copy()                                # 원탐지
det_cir = cv2.GaussianBlur(det_cir, (9, 9), 0)      # 원탐지
det_cir = cv2.cvtColor(det_cir, cv2.COLOR_BGR2GRAY) # 원탐지 이미 회색
circles = cv2.HoughCircles(det_cir, cv2.HOUGH_GRADIENT, 1, 10, param1=250, param2=20, minRadius=5, maxRadius=10)
...
if circles is not None:
    ...
    cnt = 0
    for i in circles[0, :]:
        cv2.circle(det_cir, (i[0], i[1]), i[2], (0, 0, 255), 2)  # 원 외곽 컨투어 표시.
        cnt = cnt + 1

    for i in circles[0, :]:
        cv2.circle(det_cir, (i[0], i[1]), 3, (255, 0, 40), -1)  # 원의 중심점을 표시

######################### 까지 원 탐지 부분 ###############################

img = cv2.blur(img, (3, 3)) # 원본에 blur처리.
...
cv2.createTrackbar("graybar", "Trackbars", 188, 255, nothing)  # 135  V
...
cv2.createTrackbar("bluebar_", "Trackbars", 0, 255, nothing)  #  V
cv2.createTrackbar("greenbar_", "Trackbars", 0, 255, nothing)  # V
cv2.createTrackbar("redbar_", "Trackbars", 0, 255, nothing)    # V
...
while(1):
    ...
    if len(contours) != 0:
        for contour in contours:
            if (cv2.contourArea(contour) > 150) and (cv2.contourArea(contour) < 250):  # **필요한 면적을 찾아 중심점 좌표를 저장
                cv2.drawContours(frame2, contour, -1, (0, 25, 255), 2)      # ** 컨투어 그리는 부분
                ...
                #cv2.circle(frame2, (cx_origin, cy_origin), 5, (0, 0, 255), -1)  # 중심 좌표 표시
                Rivet_center.append([cx_origin, cy_origin])

    print(Rivet_center)

    #final_mask = cv2.resize(final_mask, (1945, 1500))
    final_mask = cv2.resize(final_mask, (940, 500))
    cv2.imshow('damm', final_mask)
    #
    frame2 =cv2.resize(frame2, (940, 500))
    cv2.imshow('ori',frame2)
    #
    det_cir =cv2.resize(det_cir, (940, 500))
    cv2.imshow('det_cir',det_cir)
