...
def detect_display(img, coordList):
    ...
    for i in range(len(coordList)):
        ...
        if pix == True:
            img = cv2.circle(img, (coordList[i][0], coordList[i][1]), coordList[i][2], (255, 0, 255),
                             2)  # x좌표, y좌표, 직경값
            img = cv2.circle(img, (coordList[i][0], coordList[i][1]), 1, (0, 0, 255), -1)
            cv2.putText(img, "OK", (coordList[i][0], coordList[i][1] - 3), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2,
                        cv2.LINE_AA)
            ok_cnt = ok_cnt + 1
        else:
            img = cv2.circle(img, (coordList[i][0], coordList[i][1]), 1, (0, 0, 255), -1)
            cv2.putText(img, "NG", (coordList[i][0], coordList[i][1] - 3), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2,
                        cv2.LINE_AA)
            ng_cnt = ng_cnt + 1
