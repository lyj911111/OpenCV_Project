'''
    Choke의 각도 검출.
    Mode1 : Dipole 사각 마스크로 제거후, Choke 만 걸러냄. (조명영향이 큼)
...
def find_area(masked_img, img):
    ...
    if len(contours) != 0:
        for contour in contours:
            if (cv2.contourArea(contour) > 1000) and (
                    cv2.contourArea(contour) < 1000000):  # **필요한 면적을 찾아 중심점 좌표를 저장 (영역 제한)
...
def onlychokeFiltering(onlychoke):
    ...
    if len(contours) != 0:
        for contour in contours:
            if (cv2.contourArea(contour) > 30) and (cv2.contourArea(contour) < 10000000):  # **필요한 면적을 찾아 중심점 좌표를 저장
    ...
    if len(contours) != 0:
        ...
        for contour in contours:
            if (cv2.contourArea(contour) > 500) and (cv2.contourArea(contour) < 10000000):  # **필요한 면적을 찾아 중심점 좌표를 저장
                ...
                b = lefty
                cv2.putText(img, "y = ({})x + {}".format(a, b), (cx_origin - 200, cy_origin - 30),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 2)  # 좌표의 1차 함수방정식
                ...
                cv2.putText(img, "y = ({})x + {}".format((asum / cnt), (bsum / cnt)), (cx_origin - 200, cy_origin - 10),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2)  # 평균을 구한 일차함수

                print("리스트값", aList, bList)
                print("총합값", asum, bsum)
                print("평균 1차함수값: y = ({})x + {}".format((asum / cnt), (bsum / cnt)))  # 기울기 평균 / y절편 평균

                print("평균의 양끝 좌표 값", (0, (bsum / cnt)),
                      (cp_img.shape[1] - 1, (asum / cnt) * (cp_img.shape[1] - 1) + (bsum / cnt)))
                ...
                final_end_ptList.append([(0, int(bsum / (cnt))), (
                int(cp_img.shape[1] - 1), int(((asum / (cnt)) * cp_img.shape[1] - 1) + (bsum / (cnt))))])
                print(final_end_ptList)
                print("---")

                # 평균을 낸 값.  (오른쪽, 왼쪽)
                cv2.line(img, (0, int(bsum / (cnt))),
                         (int(cp_img.shape[1] - 1), int(((asum / (cnt)) * cp_img.shape[1] - 1) + (bsum / (cnt)))),
                         (0, 255, 255), 1)
                ...
                if flag == 1:
                    ...
                    display_final(showimg, final_end_ptList[count - 1][0][0], final_end_ptList[count - 1][0][1],
                                  final_end_ptList[count - 1][1][0], final_end_ptList[count - 1][1][1],
                                  save_function_list[count - 1])
                    print("==== flag ====")
                    flag = 0
...
def judgeImage(img):
    ...
    # cv2.imwrite('G2.png', masked_edge_img)
...
def main():
    ...
    while True:
        img = cv2.imread("./img/G1.bmp")  # 연속 이미지 취득 (비디오 프레임 가정) - 양품
