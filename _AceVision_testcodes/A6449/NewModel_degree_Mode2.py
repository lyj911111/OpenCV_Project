...
def judgeImage(img):
    ...
    if len(contours) != 0:
        for contour in contours:
            if (cv2.contourArea(contour) > 8000) and (cv2.contourArea(contour) < 9460):  # **필요한 면적을 찾아 중심점 좌표를 저장
...
def main():
    ...
    while True:
        ...
        img = cv2.imread("./img/G1_error.bmp")  # 연속 이미지 취득 (비디오 프레임 가정) - 불량1
        # img = cv2.imread("./img/degree1.bmp")  # 연속 이미지 취득 (비디오 프레임 가정) - 불량1
        # img = cv2.imread("./img/degree2.bmp")  # 연속 이미지 취득 (비디오 프레임 가정) - 불량2
        # img = cv2.imread("./img/degree3.bmp")  # 연속 이미지 취득 (비디오 프레임 가정) - 불량3
        # img = cv2.imread("./img/degree4.bmp")  # 연속 이미지 취득 (비디오 프레임 가정) - 불량4
        # img = cv2.imread("./img/degree5.bmp")  # 연속 이미지 취득 (비디오 프레임 가정) - 불량5
