...
import imutils                              # pip install imutils==0.5.2
...
from pylibdmtx.pylibdmtx import decode      # pip install pylibdmtx==0.1.7
...
def qr_image():
    ...
    return frame_cp[(point_y1):(point_y4), (point_x1):(point_x4)]
...
def leave_log(check_function):
    ...
    if (year != check_year and month != check_month and day != check_day) or check_make_log_folder != 2:
        ...
        if check_function ==0:
            ...
            f = open(store_location + today + "/barcode/log/log_%s.txt" % filename, 'w', encoding = 'utf-8')
            data = "시리얼 넘버  //  판독 시간  //  누적 판독량  //  정상  //  불량  \n"
        else:
            today = get_today()
            foldername_log = store_location + today + "/sticker" + "/log"
            make_folder(foldername_log)
            f = open(store_location + today + "/sticker/log/log_%s.txt" % filename, 'w', encoding = 'utf-8')
            data = "시리얼 넘버  //  판독 시간  //  누적 판독량  //  정상  //  불량  \n"
...
def SSDetect(frame):
    ...
    ################# 템플릿 경로 설정 ########################
    template = cv2.imread('C:/AceVision/attached file/image/template.png', 0)
    ##########################################################
...
def setting_window():
    ...
    setting.iconbitmap("aceantenna.ico")
...
def execute():
    ...
    cv2.createTrackbar("gap", "Trackbars", 3, 10, nothing)
    cv2.createTrackbar("deg", "Trackbars", 3, 5, nothing)
