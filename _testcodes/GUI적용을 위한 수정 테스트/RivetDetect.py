...
def leave_log(cam, serialnum, OK, NG):
    ...
    if year != check_year and month != check_month and day != check_day:
        print(cam + "번 카메라 새로운 로그파일 생성")
        f = open(store_location + "/" + cam + "번카메라_log_%s.txt" % filename, 'w')
    ...
    f = open(store_location + "/" + cam + "번카메라_log_%s.txt" % filename, 'a')
...
def execute(cam):
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xff == ord('a'):
            ...
            if judge != 0:
                if judge == "OK":
                    cv2.imwrite(store_location + "/PASS/" + str(serialnum) + "_" + "cam" + cam + "_" + s + "_" + str(cnt) + "_" + judge + ".jpg", frame)
                else:
                    cv2.imwrite(store_location + "/NG/" + str(serialnum) + "_" + "cam" + cam + "_" + s + "_" + str(cnt) + "_" + judge + ".jpg", frame)
            else:
                print("No data")
            ...
            leave_log(cam, serialnum, OK, NG)       # 판독값을 로그로 남김.
