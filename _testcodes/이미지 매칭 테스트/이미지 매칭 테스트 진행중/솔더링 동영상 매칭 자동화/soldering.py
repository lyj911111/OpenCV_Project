...
ROI_list = [
    [(90, 350), (130, 400)],    # 첫번째 관심영역 (start_pt, end_pt) 30 200 1200 500
    [(285, 350), (330, 400)],
    [(350, 350), (410, 400)],
    [(550, 350), (600, 400)],
    [(630, 340), (680, 400)],
...
## 추가한 template의 갯수만큼 <threshold, sensitivity> 지정.
template_setting = [
    [0.859, 300],
    [0.859, 300],
    [0.859, 300],
...
def find_loop(img, loc, color, sensitivity=300):
    ...
    for pt in zip(*loc[::-1]):
        ...
        for i in range(len(ROI_list)):
            if (pt[0] > ROI_list[i][0][0] and pt[1] > ROI_list[i][0][1]) and ((pt[0] + w_h_list[1][1] < ROI_list[i][1][0]) and (pt[1] + w_h_list[1][0] < ROI_list[i][1][1])):
                cv2.rectangle(img, pt, (pt[0] + w_h_list[1][1], pt[1] + w_h_list[1][0]), color, 1)  # 마킹
                f.add((round(pt[0] / sensitivity), round(pt[1] / sensitivity)))

        # if (pt[0] > ROI_list[0][0][0] and pt[1] > ROI_list[0][0][1]) and ((pt[0] + w_h_list[1][1] < ROI_list[0][1][0]) and (pt[1] + w_h_list[1][0] < ROI_list[0][1][1])):
        #     cv2.rectangle(img, pt, (pt[0] + w_h_list[1][1], pt[1] + w_h_list[1][0]), color, 1)  # 마킹
        #     f.add((round(pt[0] / sensitivity), round(pt[1] / sensitivity)))
        #
        # if (pt[0] > ROI_list[1][0][0] and pt[1] > ROI_list[1][0][1]) and ((pt[0] + w_h_list[1][1] < ROI_list[1][1][0]) and (pt[1] + w_h_list[1][0] < ROI_list[1][1][1])):
        #     cv2.rectangle(img, pt, (pt[0] + w_h_list[1][1], pt[1] + w_h_list[1][0]), color, 1)  # 마킹
        #     f.add((round(pt[0] / sensitivity), round(pt[1] / sensitivity)))
        #
        # if (pt[0] > ROI_list[2][0][0] and pt[1] > ROI_list[2][0][1]) and ((pt[0] + w_h_list[1][1] < ROI_list[2][1][0]) and (pt[1] + w_h_list[1][0] < ROI_list[2][1][1])):
        #     cv2.rectangle(img, pt, (pt[0] + w_h_list[1][1], pt[1] + w_h_list[1][0]), color, 1)  # 마킹
        #     f.add((round(pt[0] / sensitivity), round(pt[1] / sensitivity)))

        # 관심영역(ROI)으로 판독 제한
        # if (pt[0] > s_pt[0] and pt[1] > s_pt[1]) and ((pt[0] + w_h_list[1][1] < e_pt[0]) and (pt[1] + w_h_list[1][0] < e_pt[1])):
        #     cv2.rectangle(img, pt, (pt[0] + w_h_list[1][1], pt[1] + w_h_list[1][0]), color, 1)  # 마킹
        #     f.add((round(pt[0] / sensitivity), round(pt[1] / sensitivity)))
        #
    found_count = len(f)
...
def execute():
    ...
    while (True):
        ...
        # result = cv2.rectangle(result, ROI_list[0][0], ROI_list[0][1], (0, 0, 255), 3)
        # result = cv2.rectangle(result, ROI_list[1][0], ROI_list[1][1], (0, 255, 0), 3)
        # result = cv2.rectangle(result, ROI_list[2][0], ROI_list[2][1], (255, 0, 0), 3)
