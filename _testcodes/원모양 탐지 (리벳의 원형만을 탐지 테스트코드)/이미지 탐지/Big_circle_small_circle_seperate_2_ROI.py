...
def houghCircle():
    ...
    result = cv2.imread('../123.jpg')    # 원형이 있는 이미지를 올림.
...
###########################################################################################################################
    ...
    circles = cv2.HoughCircles(imgray, cv2.HOUGH_GRADIENT, 0.5, 10, param1=70, param2=16, minRadius=8, maxRadius=30)
    ...
    if circles is not None:
        ...
        for i in circles[0, :]:
            ...
            for j in range(len(ROI_list)):
                if (i[0] > ROI_list[j][0][0] and i[0] < ROI_list[j][1][0]) and (i[1] > ROI_list[j][0][1] and i[1] < ROI_list[j][1][1]):  # ROI 관심영역 내에서 찾아냄.
                    ...
                    length = len(whole_circle_list)-1
                    whole_circle_tuple.append(tuple(whole_circle_list[length]))
        ...
        for z in range(len(whole_circle_tuple)):
            cv2.circle(result, whole_circle_tuple[z], 12, (0, 0, 255), 2)  # 원 외곽 컨투어 표시.
...
#####################################################################################################
    # 작은 원을 탐지
    sml_circles = cv2.HoughCircles(imgray, cv2.HOUGH_GRADIENT, 0.5, 10, param1=70, param2=10, minRadius=8, maxRadius=10)
        ...
        for i in sml_circles[0, :]:
            ...
            for j in range(len(ROI_list_small)):
                if (i[0] > ROI_list_small[j][0][0] and i[0] < ROI_list_small[j][1][0]) and (i[1] > ROI_list_small[j][0][1] and i[1] < ROI_list_small[j][1][1]):  # ROI 관심영역 내에서 찾아냄.
                    ...
                    length = len(whole_circle_list) - 1                             # list를 tuple로 바꾸기 위해 index값 추출.
                    whole_circle_tuple.append(tuple(whole_circle_list[length]))     # 그 index를 이용해서 tuple로 변경.
                    # print("튜플로 변환된", whole_circle_tuple)

        for z in range(len(whole_circle_tuple)):
            cv2.circle(result, whole_circle_tuple[z], 8, (0, 0, 255), 2)  # 원 외곽 컨투어 표시.
            cv2.circle(result, whole_circle_tuple[z], 3, (255, 0, 40), -1)  # 원의 중심점을 표시
