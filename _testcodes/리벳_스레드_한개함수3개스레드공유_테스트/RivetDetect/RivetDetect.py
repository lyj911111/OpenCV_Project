...
def leave_log(x, serialnum, OK, NG):
    ...
    if year != check_year and month != check_month and day != check_day:
        print(x + "번 카메라 새로운 로그파일 생성")
        f = open(store_location + "/" + x + "번카메라_log_%s.txt" % filename, 'w')
    ...
    f = open(store_location + "/" + x + "번카메라_log_%s.txt" % filename, 'a')
    time = str(year) + "_" + str(month) + "_" + str(day) + "_" + str(hour) + "_" + str(minute) + "_" + str(sec)
    data = str(serialnum) + "       //        " + str(time) + "  //        " + str(accum) + "          //     " + str(
        OK) + "    //    " + str(NG) + '\n'
    f.write(data)
    f.close()

# 사용할 카메라를 인자로 받음
def execute(x, serialnum):
    ...
    while True:
        ...
        if Start_Rivet_flag == 0:   # 시작할때 한번만 작동 플레그.
            ...
            ##### 자동 좌표값 저장하기 #####
            print(x + " 저장된 리벳의 좌표:", Rivet_center)  # 자동 저장된 중심점값 출력
            ...
            cv2.imshow('init_location' + x + '.jpg', frame)    # 이미지 확인용.
            cv2.imwrite('init_location' + x + '.jpg', frame)   # 처음 이미지 캡쳐후 저장.
            ##############################
        ...
        cv2.imshow('Frame' + x, frame)                      # 원본
        #cv2.imshow('result' + x, final_mask)                # 필터링후
        #cv2.imshow('reverse + x', reverse)                  # 반전 (픽셀값을 찍어보기 위해 흑백)
        #cv2.imshow('location_check + x', reverse_copy)      # 리벳위치 픽셀체크 위치 확인용
        ...
        if cv2.waitKey(1) & 0xff == ord('a'):
            ...
            leave_log(x, serialnum, OK, NG)       # 판독값을 로그로 남김.
