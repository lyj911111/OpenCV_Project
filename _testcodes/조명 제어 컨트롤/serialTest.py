    ...
    def lightExecute():
        while True:
            if ser.readable():
                ...
                DATA = DATA.encode()  # DATA값 전송. 아스키코드
                ser.write(DATA)  # 데이터 전송
                ...
                reply = res.decode()[:len(
                    res) - 2]  # bytes를 decode 해서 str로 변환, 자동으로 포함되어있는 개행을 제거하기 위해 [:len(res)-2] 를 추가함 : 즉, \r\n 을 제거함.
...
if __name__=="__main__":
    ...
    STX, DATA, ETX = seperateControl(3, 255)

    # 전체 조명 제어 함수로부터 start값, data값, end값을 return
    STX1, DATA1, ETX1 = wholeControl(50)
